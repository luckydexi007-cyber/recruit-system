...
async function handleApi(method, pathname, body, req, res) {
  ...
  if (method === 'POST' && pathname === '/api/login') {
    if (!isStr(body.studentId) || !isStr(body.password)) { send(res, 400, { error: '参数类型错误' }); return true; }
    const sid = String(body.studentId || '').trim();
    const password = String(body.password || '');
  ...
  if (method === 'POST' && pathname === '/api/terminal/login') {
    if (!isStr(body.username) || !isStr(body.password)) { send(res, 400, { error: '参数类型错误' }); return true; }
  ...
  if (method === 'POST' && pathname === '/api/resumes') {
    ...
    if (body.file && body.file.data) {
      ...
      const rawData = String(body.file.data);
      const mm = rawData.match(/^data:([^;,]+)[;,]/);
      const actualMime = mm ? mm[1].toLowerCase() : '';
      const ALLOWED_MIME = ['application/pdf', 'image/png', 'image/jpeg', 'image/gif', 'image/webp', 'image/bmp', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];
      if (!ALLOWED_MIME.includes(actualMime)) { send(res, 400, { error: '仅支持 PDF、图片或 Word 文档' }); return true; }
      const safeName = String(body.file.name || 'attachment').replace(/[^\w.\u4e00-\u9fa5 -]/g, '_').slice(0, 120);
      file = { name: safeName, size: Number(body.file.size) || 0, type: actualMime, data: rawData };
    }
  ...
  if (method === 'DELETE' && m) {
    ...
    const owner = idx >= 0 && session.userId && db.resumes[idx].userId === session.userId;
    if (idx < 0 || (!session.isAdmin && !owner)) { send(res, 404, { error: '简历不存在' }); return true; }
